...
- testes: suítes unitárias/integração no Server e Web, E2E integrado opcional, benchmarks e budget.
...
- schema: `server/database/postgresMigrations.js` no `araru-server`;
- Docker: `docker-compose.yml` e Dockerfiles.

## Não implementado

S3/R2, workers externos, escala horizontal, OpenAPI completo e clientes nativos continuam planejados. PostgreSQL, Redis, usuários e perfis já estão implementados.
